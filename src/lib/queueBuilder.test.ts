...
function createMockCard(id: string): Flashcard {
  return {
    id,
...
function createMockProgress(cardKey: string, daysFromNow: number): CardProgress {
...
describe('queueBuilder', () => {
  describe('buildStudyQueue', () => {
    it('should prioritize due reviews over new cards', () => {
      ...
      const queue = buildStudyQueue(cards, progress, DEFAULT_CONFIG)
    ...
    it('should sort reviews by days overdue when prioritizeOverdue is true', () => {
      ...
      const queue = buildStudyQueue(cards, progress, { ...DEFAULT_CONFIG, prioritizeOverdue: true })
    ...
    it('should respect review cards per session limit', () => {
      const cards = Array.from({ length: 30 }, (_, i) => createMockCard(`card-${i}`))
      ...
      const queue = buildStudyQueue(cards, progress, { ...DEFAULT_CONFIG, reviewCardsPerSession: 10 })
    ...
    it('should respect new cards per session limit', () => {
      ...
      const queue = buildStudyQueue(cards, progress, { ...DEFAULT_CONFIG, newCardsPerSession: 5 })
    ...
    it('should handle empty card list', () => {
      const queue = buildStudyQueue([], {}, DEFAULT_CONFIG)
    ...
    it('should handle empty progress (all new cards)', () => {
      ...
      const queue = buildStudyQueue(cards, {}, DEFAULT_CONFIG)
  ...
  describe('getRecommendedSessionSize', () => {
    it('should prioritize reviews over new cards', () => {
      const cards = Array.from({ length: 50 }, (_, i) => createMockCard(`card-${i}`))
    ...
    it('should handle no due reviews', () => {
      const cards = Array.from({ length: 20 }, (_, i) => createMockCard(`card-${i}`))
      const progress: Record<string, CardProgress> = {}
